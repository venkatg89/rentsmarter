...
const SubscriptionServicesSlider = (props) => {
  ...
  let [initialItemValue, setInitialItemValue] = useState(subscriptionservicesslider);
  ...
  const changeValue = (value) => {
    if (firstTimeThrough) {
      ...
      // cdispatch(c_changesubscriptionservicesslider(value));
  ...
  const positiveSliderMove = (value, beginSlideValue) => {
    ...
    if (initialTotalValue + diff > 5000) {
      ...
      cdispatch(c_changesubscriptionservicesslider(initialItemValue + diff));
    
      dispatch(changesubscriptionservices(initialItemValue + diff))
  ...
  const slidingComplete = (value: number) => {
    cdispatch(c_changesubscriptionservicesslider(value));
      ...
      dispatch(changesubscriptionservices(value))
    ...
    async () =>
      await analytics().logEvent("ItemizedExp_Subscription_Slider_Move", {
        id: 60000005,
        event: "slider-subscriptionservices",
        description: ["used subscription services slider", { value }],
      });
  ...
  const getBoolEditableValue = () => {
    let editable: boolean = subscriptionservices_editable;
  ...
  useEffect(() => {
    ...
    cdispatch(c_changesubscriptionservicestext(calculatorUIData.subscriptionservices));
    cdispatch(c_changesubscriptionservicesslider(calculatorUIData.subscriptionservices));
    ...
    <ViewContainer>
      <SliderWrapper>
        <Slider
          ...
          // })}
          value={subscriptionservicestext} // set by text input
...
export default connect(select)(SubscriptionServicesSlider);
