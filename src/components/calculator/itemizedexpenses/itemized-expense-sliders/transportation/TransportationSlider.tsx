...
const TransportationSlider = (props) => {
  ...
  let [initialItemValue, setInitialItemValue] = useState(transportationslider);
  ...
  const changeValue = (value) => {
    if (firstTimeThrough) {
      ...
      // cdispatch(c_changetransportationslider(value));
  ...
  const positiveSliderMove = (value, beginSlideValue) => {
    ...
    if (initialTotalValue + diff > 5000) {
      ...
      cdispatch(c_changetransportationslider(initialItemValue + diff));
      
      dispatch(changetransportation(initialItemValue + diff))
  ...
  const slidingComplete = (value: number) => {
    ...
    async () =>
      await analytics().logEvent("ItemizedExp_Transpo_Slider_Move", {
        id: 60000006,
        event: "slider-transportation",
        description: ["used transportation slider", { value }],
      });
  ...
  const getBoolEditableValue = () => {
    let editable: boolean = transportation_editable;
  ...
  useEffect(() => {
    ...
    cdispatch(c_changetransportationtext(calculatorUIData.transportation));
    cdispatch(c_changetransportationslider(calculatorUIData.transportation));
  }, []);
    ...
    <ViewContainer>
      <SliderWrapper>
        <Slider
          ...
          // })}
          value={transportationtext} // set by text input
...
export default connect(select)(TransportationSlider);
