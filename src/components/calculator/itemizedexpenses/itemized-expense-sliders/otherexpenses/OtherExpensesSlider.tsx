...
const OtherExpensesSlider = (props) => {
  ...
  let [initialItemValue, setInitialItemValue] = useState(otherexpensesslider);
  ...
  const positiveSliderMove = (value, beginSlideValue) => {
    ...
    if (initialTotalValue + diff > 5000) {
      ...
      cdispatch(c_changeotherexpensesslider(initialItemValue + diff));
     
      dispatch(changeotherexpenses(initialItemValue + diff))
  ...
  const slidingComplete = (value: number) => {
    cdispatch(c_changeotherexpensesslider(value));
      ...
      dispatch(changeotherexpenses(value))
    ...
    async () =>
      await analytics().logEvent("ItemizedExp_OtherExp_Slider_Move", {
        id: 60000004,
        event: "slider-otherexpenses",
        description: ["used other expenses slider", { value }],
      });
  ...
  const getBoolEditableValue = () => {
    let editable: boolean = otherexpenses_editable;
  ...
  useEffect(() => {
    ...
    cdispatch(c_changeotherexpensestext(calculatorUIData.otherexpenses));
    cdispatch(c_changeotherexpensesslider(calculatorUIData.otherexpenses));
  }, []);
    ...
    <ViewContainer>
      <SliderWrapper>
        <Slider
          ...
          // })}
          value={otherexpensestext} // set by text input
...
export default connect(select)(OtherExpensesSlider);
