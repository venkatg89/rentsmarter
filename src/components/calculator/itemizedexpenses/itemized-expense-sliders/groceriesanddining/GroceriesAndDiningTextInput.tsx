...
import {
  ...
  c_groceriesanddiningvalueiseditable,
...
const GroceriesAndDiningTextInput = (props) => {
  ...
  const OnChangeText = (value) => {
    // // //console.log("OnChangeText GroceriesAndDining()  function");
    ...
    analytics().logEvent("ItemizedExp_GroAndDine_Text_Change", {
      id: 91000008,
      event: "text-input-changed-groceriesanddinnig",
      description: ["used groceries and dining text input", { cleanedNumber }],
    });
    ...
    if (cleanedNumber > initialItemValue) {
      ...
      if (Number(initialTotalValue + difference) <= CalculatorState.grossIncomeRange / 2) {
        ...
        cdispatch(c_changegroceriesanddiningslider(cleanedNumber));
        cdispatch(c_changegroceriesanddiningtext(cleanedNumber));
        dispatch(changegroceriesanddining(groceriesanddiningtext));
      } else {
        ...
        cdispatch(c_changegroceriesanddiningslider(initialItemValue));
        cdispatch(c_changegroceriesanddiningtext(initialItemValue));
        dispatch(changegroceriesanddining(initialItemValue));
      }
    }
    ...
    if (initialItemValue >= cleanedNumber) {
      ...
      if (Number(initialTotalValue - difference) >= 0) {
        ...
        cdispatch(c_changegroceriesanddiningslider(0));
        cdispatch(c_changegroceriesanddiningtext(0));
        dispatch(changegroceriesanddining(0));
      }
    }
  };

  const OnChange = ({ nativeEvent: { eventCount, target, text } }) => {
    //console.log("OnChange() groceriesanddining function");
    // //console.log(value);
    dispatch(changegroceriesanddining(groceriesanddiningslider)); // not sure this is needed
  };

  const OnEndEditing = () => {
    // // //console.log("OnEndEditing() groceriesanddining function");

     
    cdispatch(c_groceriesanddiningvalueiseditable(false));
    dispatch(changegroceriesanddining(groceriesanddiningtext));
  ...
  const OnFocus = () => {
    ...
    cdispatch(c_groceriesanddiningvalueiseditable(true));
  ...
  const getCleanedValueFromSlider = () => {
    ...
    return String(groceriesanddiningslider);
  };
...
export default connect(select)(GroceriesAndDiningTextInput);
