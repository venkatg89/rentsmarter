...
type props = {listData: any | undefined ; marginVertical: number | undefined ; saveProperty: boolean | undefined; OnSavePropertyClick: () => void | undefined;}

const PropertyCard: React.FC<props> = ({ listData, marginVertical, saveProperty, OnSavePropertyClick}) => {
    let imageData =[]
    if (listData?.Media.length > 0){
        {listData && listData.Media && listData.Media.map((item: { MediaURL: any; }, index: any) => {
            imageData.push ({
            url: item?.MediaURL ?? ''
        })
    })}
    ...
    return (
        <View style={{
            ...
            <View style={{
                ...
                <View style = {{ right: 0, left: Platform.OS === 'ios' ? 280 : 300, bottom: 30, }}>
                    <TouchableOpacity 
                    onPress={() => OnSavePropertyClick()}
                    >
                     {saveProperty?
                        <Image
                ...
                <View>
                    <View style={{
                        ...
                        <Text style={{
                            ...
                            color: "#121212",
                            marginTop: 10,
                        }}>{listData?.UnparsedAddress ?? ''}, {listData?.PostalCity ?? ''}, {listData?.StateOrProvince ?? ''}, {listData?.PostalCode ?? ''}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', marginLeft: marginVertical ? 30 : 15, marginTop: 10, marginBottom: 20 }}>
                        <Text style={styles.planText}>Plan E2 | </Text>
                        <Text style={styles.planText}>{listData?.BedroomsTotal ?? ''} Beds | </Text>
                        <Text style={styles.planText}>{listData?.BathroomsTotalInteger ?? ''} Baths | </Text>
